/* 
* @Author: Marte
* @Date:   2019-04-26 09:15:50
* @Last Modified by:   Marte
* @Last Modified time: 2019-04-26 09:43:43
*/
// * 回到顶部(讲过)
...
// * 吸顶菜单(讲过)
       ...
       //  * 参数二：数组，吸顶前和吸顶后的类名
       //  * 目标节点的父节点不能为body
function fixedTop(node,classname) {
    let parent = node.parentNode;
    let stand = document.createElement(node.tagName);
    stand.className = classname[0];
    let itop = node.offsetTop;
    let isok = false;
    window.onscroll = function() {
        //关于垂直滚动的完全兼容
        ...
        if (itop < scrollY) {
            parent.insertBefore(stand,node);
            node.className = classname[1];
            isok = true;
        } else {
            if (isok) {
                parent.removeChild(parent.getElementsByClassName(classname[0])[0]);
                node.className = classname[0];
                isok = false;
            }
        }
    }
